...
class Node:
    def __init__(self, G=0, H=0, coordinate=None, parent=None):
        self.G, self.H = G, H
        self.F = (F := G + H)
        self.parent, self.coordinate = parent, coordinate
...
def heuristic_cost(current_pos, goal_pos):
    x_side_dx = abs(current_pos[0] - goal_pos[0])
    y_side_dy = abs(current_pos[1] - goal_pos[1])
    heuristic_cost = x_side_dx + y_side_dy
    return heuristic_cost


def movement_cost(fixed_node, target_coordinate):
    dx = abs(fixed_node.coordinate[0] - target_coordinate[0])
    dy = abs(fixed_node.coordinate[1] - target_coordinate[1])
    distance = math.hypot(dx, dy)
    total_cost = fixed_node.G + distance
    return total_cost


def generate_boundary_and_obstacles(start_point, end_point, top_vertex, bottom_vertex, num_obstacles):
    ...
    ob_x = np.random.randint(bottom_vertex[0] + 1,
                             top_vertex[0], num_obstacles).tolist()
    ob_y = np.random.randint(bottom_vertex[1] + 1,
                             top_vertex[1], num_obstacles).tolist()
    x = ax + bx + cx + dx
    y = ay + by + cy + dy
    obstacle = np.vstack((ob_x, ob_y)).T.tolist()
    obstacle = [coor for coor in obstacle if coor != start_point and coor != end_point]
...
def searching_control(start, end, bound, obstacle):
    origin = Node(coordinate=start, H=heuristic_cost(start, end))
    goal = Node(coordinate=end, H=heuristic_cost(end, start))
...
def main(obstacle_number=1500):
    ...
    bound, obstacle = generate_boundary_and_obstacles(start, end, top_vertex,
                                             bottom_vertex,
                                             obstacle_number)
